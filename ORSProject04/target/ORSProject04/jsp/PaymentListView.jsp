	...
	<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
		...
		<div
			...
			<div class="d-flex gap-2">
				<a href="<%=ORSView.PAYMENT_REPORT_CTL%>" target="_blank"
			...
			<a href="<%=ORSView.PAYMENT_CTL%>"
				...
				class="bi bi-plus-circle me-1"></i> <%=ms.get("payment.add")%>
			</a>
		...
		<form action="<%=ORSView.PAYMENT_LIST_CTL%>" method="POST">
			...
			<!-- Search -->
			<div
				class="p-3 bg-light border-bottom d-flex gap-2 align-items-center">

				<input type="text" name="upi" class="form-control"
					style="max-width: 250px;" placeholder="Search by UPI">
				...
				<button type="submit" name="operation"
					value="<%=BaseCtl.OP_DELETE%>" class="btn btn-danger ms-auto"
				...
				<table class="table table-hover align-middle mb-0">
					...
					<tbody>
						...
						<tr>
							...
							<td><%=bean.getNetBanking()%></td>

							<td><%=bean.getWallet()%></td>

							<td><%=bean.getPaymentStatus()%></td>

							<td><a href="<%=ORSView.PAYMENT_CTL%>?id=<%=bean.getId()%>"
								class="btn btn-sm btn-outline-primary"> <i
									class="bi bi-pencil"></i> <%=ms.get("label.edit")%>
